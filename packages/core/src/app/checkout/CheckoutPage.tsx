...
const Checkout = ({
    ...
    const getPreferredStepType = useCallback((): CheckoutStepType | undefined => {
        const shippingStep = stepsRef.current.find(
            (step) => step.type === CheckoutStepType.Shipping && step.isRequired && !step.isComplete,
        );

        if (shippingStep) {
            return shippingStep.type;
        }

        return stepsRef.current.find(
            (step) => step.isRequired && !step.isComplete && step.type !== CheckoutStepType.Customer,
        )?.type;
    }, []);
    ...
    const navigateToNextIncompleteStep = useCallback(
        (options?: { isDefault?: boolean }): void => {
            ...
            if (!activeStep) {
                const preferredStepType = getPreferredStepType();

                if (preferredStepType) {
                    navigateToStep(preferredStepType, options);
                }
        ...
        [analyticsTracker, getPreferredStepType, navigateToStep],
    );
    ...
    const handleReady = useCallback((): void => {
        ...
        if (preferredStepType) {
            navigateToStep(preferredStepType, { isDefault: true });

            return;
        }

        navigateToNextIncompleteStep({ isDefault: true });
    }, [getPreferredStepType, navigateToNextIncompleteStep, navigateToStep]);
                    ...
                    <>
                        <div className="layout-main">
                            ...
                            <ol className="checkout-steps">
                                {stepsRef.current
                                    .filter((step) => step.isRequired && step.type !== CheckoutStepType.Customer)
                                    .map((step) =>
                                        renderStep({
                                            ...step,
