...
function Shipping({
    ...
    const handleSingleShippingSubmit = async (values: SingleShippingFormValues) => {
        const updatedShippingAddress = values.shippingAddress
            ? mapAddressFromFormValues(values.shippingAddress)
            : undefined;
        const promises: Array<Promise<CheckoutSelectors>> = [];
        const hasRemoteBilling = hasRemoteBillingFn(methodId);

        const shippingAddressToSave = updatedShippingAddress
            ? { ...updatedShippingAddress, ...(values.email ? { email: values.email } : {}) }
            : undefined;

        if (
            !isEqualAddress(shippingAddressToSave, shippingAddress) ||
            getShouldSaveAddress(shippingAddress) !== getShouldSaveAddress(shippingAddressToSave)
        ) {
            promises.push(updateShippingAddress(shippingAddressToSave || {}));
        }

        if (values.billingSameAsShipping && shippingAddressToSave && !hasRemoteBilling) {
            if (!isEqualAddress(shippingAddressToSave, billingAddress)) {
                promises.push(updateBillingAddress(shippingAddressToSave));
            }
        }
