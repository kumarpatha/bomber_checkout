...
const PaymentBillingFormComponent = ({
    ...
    const billingAddressWithEmail = billingAddress as (Address & { email?: string }) | undefined;
    ...
    const ensureBillingAddressSaved = useCallback(async (): Promise<boolean> => {
        ...
        try {
            const { billingSameAsShipping: _billingSameAsShipping, ...billingValues } =
                valuesRef.current;
    ...
    useEffect(() => {
        if (isGuest && billingAddressWithEmail?.email && values.email !== billingAddressWithEmail.email) {
            void setFieldValue('email', billingAddressWithEmail.email);
        }
    }, [billingAddressWithEmail?.email, isGuest, setFieldValue, values.email]);
    ...
    const handleBillingFormChange = useCallback((fieldName: string, value: string | string[]) => {
        if (fieldName === 'email') {
            valuesRef.current = {
                ...valuesRef.current,
