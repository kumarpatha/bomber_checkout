...
describe('loadFiles', () => {
    ...
    it('falls back to loading the manifest from the public path when MANIFEST_JSON is unavailable', async () => {
        delete (global as any).MANIFEST_JSON;

        const fetchMock = jest.fn().mockResolvedValue({
            ok: true,
            json: jest.fn().mockResolvedValue(manifestJson),
        });
        ...
        expect(fetchMock).toHaveBeenCalledWith('https://cdn.foo.bar/manifest.json', {
