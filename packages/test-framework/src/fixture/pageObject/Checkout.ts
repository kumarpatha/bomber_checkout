...
export class Checkout {
    ...
    async completeCustomerStepAsGuest(email = `test@example.com`): Promise<void> {
        const customerGuestForm = this.page.locator('[data-test="checkout-customer-guest"]');
        const shippingHeading = this.page.locator('[data-test="shipping-address-heading"]');
        const shippingEmail = this.page.locator('#shipping-email');

        if (await customerGuestForm.isVisible().catch(() => false)) {
            await customerGuestForm.locator('#email').fill(email);
            await this.page.locator('[data-test="customer-continue-as-guest-button"]').click();

            return;
        }

        await shippingHeading.waitFor({ state: 'visible' });

        if (await shippingEmail.isVisible().catch(() => false)) {
            await shippingEmail.fill(email);
        }
    }

    async completeCustomerStep(email: string, password: string): Promise<void> {
        const customerGuestForm = this.page.locator('[data-test="checkout-customer-guest"]');
        const customerReturningForm = this.page.locator('[data-test="checkout-customer-returning"]');
        const shippingHeading = this.page.locator('[data-test="shipping-address-heading"]');

        if (await customerGuestForm.isVisible().catch(() => false)) {
            await this.page.locator('#checkout-customer-login').click();
            await customerReturningForm.waitFor({ state: 'visible' });
            await customerReturningForm.locator('#email').fill(email);
            await customerReturningForm.locator('#password').fill(password);
    ...
    async completeBillingAddressForm(address: Address): Promise<void> {
        ...
        await this.fillAddressForm({ formId: '#checkoutBillingAddress', address, addressAppendText: '' });
